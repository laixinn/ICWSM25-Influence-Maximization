...
class TensorLastMessageAggregator(MessageAggregator):
  def __init__(self, device):
    super(TensorLastMessageAggregator, self).__init__(device)

  def aggregate(self, node_ids, messages):
    """Only keep the last message for each node"""
    unique_node = torch.unique(node_ids)
    unique_node_index = torch.zeros(len(messages)).bool().to(self.device)
    unique_node_index[unique_node] = True
    _index = (messages[:, -1] != -1) & unique_node_index

    to_update_node_ids = _index.nonzero()[:, 0]
    unique_messages = messages[_index, :-1] if len(to_update_node_ids) > 0 else []
    unique_timestamps = messages[_index, -1] if len(to_update_node_ids) > 0 else []
...
def get_message_aggregator(aggregator_type, device):
  ...
  elif aggregator_type == 'lastTensor':
    return TensorLastMessageAggregator(device=device)
